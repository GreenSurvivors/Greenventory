...
public final class SyncInv extends JavaPlugin {
        ...
        runSync(() -> {
            ...
            try {
                ...
                if (shouldSync(SyncType.DIENSTMODUS) && data instanceof PlayerDataDienstmodus) {
                    DienstmodusApi.setPartialData(((PlayerDataDienstmodus)data).dienstmodusData());
                }
    ...
    public PlayerData getData(Player player) {
        byte @Nullable [] tempPersistentData = null;
        if (shouldSync(SyncType.PERSISTENT_DATA)) {
            PersistentDataContainer pdc = player.getPersistentDataContainer();
            try {
                tempPersistentData = pdc.serializeToBytes();
            } catch (IOException e) {
        ...
        final byte @Nullable [] finalPersistentData = tempPersistentData; // can't assign a final variable in a try block

        PlayerData data;
        if (shouldSync(SyncType.DIENSTMODUS)) {
            data = DienstmodusApi.getPartialData(player.getUniqueId())
                .thenApply(dmData ->
                    (PlayerData)new PlayerDataDienstmodus(player, getLastSeen(player.getUniqueId(), player.isOnline()), finalPersistentData, dmData)
                ).exceptionally(ex -> {
                    this.getLogger().log(Level.SEVERE, "Couldn't load Dienstmodus data for " + player.getName() + ", uuid: " + player.getUniqueId(), ex);

                    return new PlayerData(player, getLastSeen(player.getUniqueId(), player.isOnline()), finalPersistentData);
                }).join();
        } else {
            data = new PlayerData(player, getLastSeen(player.getUniqueId(), player.isOnline()), finalPersistentData);
        }
